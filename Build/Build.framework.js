...
// end include: preamble.js
  ...
  function _Analytics_Beacon(urlPtr, apiKeyPtr, bodyPtr) {
      var url = UTF8ToString(urlPtr);
      var apiKey = UTF8ToString(apiKeyPtr);
      var body = UTF8ToString(bodyPtr);
      try {
        var full = url + (url.indexOf('?') >= 0 ? '&' : '?') + 'apikey=' + encodeURIComponent(apiKey);
        var blob = new Blob([body], { type: 'application/json' });
        var ok = navigator.sendBeacon(full, blob);
        console.log('[Analytics] beacon queued:', ok, '(' + body.length + ' bytes)');
      } catch (e) {
        console.error('[Analytics] beacon failed:', e);
      }
    }
  ...
  function _LocalStorage_FreeString(ptr) {
      _free(ptr);
    }

  function _LocalStorage_GetItem(keyPtr) {
      var key = UTF8ToString(keyPtr);
      try {
        var value = localStorage.getItem(key);
        if (value === null) {
          console.log('[LocalStorage] GetItem: key not found:', key);
          return 0;
        }
        console.log('[LocalStorage] GetItem OK:', key, '(' + value.length + ' chars)');
  ...
  function _LocalStorage_HasKey(keyPtr) {
      var key = UTF8ToString(keyPtr);
      try {
        var exists = localStorage.getItem(key) !== null;
        console.log('[LocalStorage] HasKey:', key, exists);
        return exists ? 1 : 0;
      } catch (e) {
        console.error('[LocalStorage] HasKey failed:', key, e);
        return 0;
      }
    }

  function _LocalStorage_RemoveItem(keyPtr) {
      var key = UTF8ToString(keyPtr);
      try {
  ...
  function _LocalStorage_SetItem(keyPtr, valuePtr) {
      var key = UTF8ToString(keyPtr);
      var value = UTF8ToString(valuePtr);
      try {
        localStorage.setItem(key, value);
        console.log('[LocalStorage] SetItem OK:', key, '(' + value.length + ' chars)');
      } catch (e) {
        console.error('[LocalStorage] SetItem failed:', key, e);
      }
    }

  function _TidaflipGetSafeAreaInsets(outPtr) {
      var probe = Module.__tfSafeAreaProbe;
      if (!probe) {
        ...
        Module.__tfSafeAreaProbe = probe;
      }
      ...
      // env() values are CSS px; Unity's Screen.* are device px. Derive the ratio from the
      // canvas Unity actually rendered into (robust to config.devicePixelRatio overrides),
      // falling back to window.devicePixelRatio.
      var canvas = Module.canvas || document.querySelector('#unity-canvas');
      var scale = window.devicePixelRatio || 1;
      if (canvas) {
        var rect = canvas.getBoundingClientRect();
        if (rect.width > 0 && canvas.width > 0) scale = canvas.width / rect.width;
      }
  
      var base = outPtr >> 2;
      HEAPF32[base + 0] = l * scale;
      HEAPF32[base + 1] = r * scale;
      HEAPF32[base + 2] = t * scale;
      HEAPF32[base + 3] = b * scale;
    }

  function _TidaflipIsApplePlatform() {
      var ua = navigator.userAgent || '';
      var platform = navigator.platform || '';
      var isIOS = /iPhone|iPad|iPod/.test(ua);
      var isMac = /Mac/.test(platform) || /Mac/.test(ua);
      return (isIOS || isMac) ? 1 : 0;
    }

  function _TidaflipIsTopLevel() {
      try {
...
var wasmImports = {
  "Analytics_Beacon": _Analytics_Beacon,
  ...
  "LocalStorage_FreeString": _LocalStorage_FreeString,
  "LocalStorage_GetItem": _LocalStorage_GetItem,
  "LocalStorage_HasKey": _LocalStorage_HasKey,
  "LocalStorage_RemoveItem": _LocalStorage_RemoveItem,
  "LocalStorage_SetItem": _LocalStorage_SetItem,
  "TidaflipGetSafeAreaInsets": _TidaflipGetSafeAreaInsets,
  "TidaflipIsApplePlatform": _TidaflipIsApplePlatform,
  "TidaflipIsTopLevel": _TidaflipIsTopLevel,
